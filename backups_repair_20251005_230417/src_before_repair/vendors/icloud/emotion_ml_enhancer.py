...
from jeffrey.core.emotions.core.emotion_prompt_detector import EmotionPromptDetector
...
class EmotionMLEnhancer:
    ...
    def __init__(self, history_size: int = 50, history_file: str | None = None) -> None:
        """
    ...
    def _compat_intensity(self, text: str) -> float:
        """Compatibilité intensité (zéro-invention : adaptation dans consommateur)."""
        det = getattr(self, "detector", None)
        if det is None:
            return 0.0

        # 1. Méthode native si dispo
        gi = getattr(det, "get_emotion_intensity", None)
        if callable(gi):
            try:
                return float(gi(text) or 0)
            except:
                pass

        # 2. Calcul dynamique
        da = getattr(det, "detect_all_emotions", None)
        if callable(da):
            try:
                scores = da(text)
                if scores:
                    total = sum(float(v) for v in scores.values() if v)
                    return max(0.0, min(1.0, total / max(len(scores), 1)))
            except:
                pass
        return 0.0
    ...
    def detect_emotion_enhanced(self, text: str, user_id: str = "default") -> dict[str, Any]:
        ...
        main_emotion = max(adjusted_scores.items(), key=lambda x: x[1])[0] if adjusted_scores else None

        return {
            "emotion": main_emotion,
            "scores": adjusted_scores,
            "confidence": confidence,
            "intensity": self._compat_intensity(text),
            "context": {
    ...
    def _predict_next_emotion(self, current_scores: dict[str, float]) -> str | None:
        """
    ...
    def _generate_recommendations(self, dominant_emotion: str, volatility: float) -> list[str]:
        ...
        if volatility > 0.7:
            recommendations.append("🌊 Vos émotions sont très changeantes. Un moment de pause pourrait aider.")
        ...
        return recommendations if recommendations else ["🌟 Continuez à partager vos émotions avec moi."]
    ...
    def enhance_detection(self, text: str, base_detection: dict[str, Any]) -> dict[str, Any]:
        """
        Amélioration de la détection existante (compatibilité).
        """
        return self.detect_emotion_enhanced(text)

    def learn_from_feedback(self, text: str, emotions: dict[str, Any], feedback: dict[str, Any]):
        """
        Apprentissage depuis le feedback (compatibilité).
        """
        # Implémenter apprentissage depuis feedback si nécessaire
        pass
