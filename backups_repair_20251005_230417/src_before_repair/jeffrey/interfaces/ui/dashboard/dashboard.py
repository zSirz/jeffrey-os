...
from typing import Any, Dict, List, Optional, Union
    ...
    def load_data(self):
        ...
        if eco_scores_path.exists():
            try:
                with open(eco_scores_path, "r", encoding="utf-8") as f:
                    self.eco_scores = json.load(f)
                logger.debug(
                    f"Chargement des écoscores pour {len(self.eco_scores)} modèles")
            except json.JSONDecodeError as e:
                logger.error(
                    f"Erreur de format JSON dans le fichier d'écoscores: {e}", exc_info=True
                )
                self.eco_scores = {}
            except Exception as e:
                logger.error(
                    f"Erreur lors du chargement des écoscores: {e}",
                    exc_info=True)
                self.eco_scores = {}
        ...
        if registry_path.exists():
            try:
                ...
                logger.debug(
                    f"Chargement du registre IA réussi: {len(self.ia_registry.get('models', []))} modèles"
                )
            except ImportError:
                logger.error(
                    "Module yaml non disponible. Installation nécessaire: pip install pyyaml"
                )
        ...
        if comparisons_path.exists():
            try:
                ...
                logger.debug(
                    f"Chargement de {len(self.comparisons)} comparaisons depuis {comparisons_path}"
                )
            except json.JSONDecodeError as e:
                logger.error(
                    f"Erreur de format JSON dans le fichier de comparaisons: {e}", exc_info=True
                )
                self.comparisons = []
            except Exception as e:
    ...
    def generate_html(self):
        ...
        ecoscore_labels = [
            info.get("eco_label", "Non défini") for model, info in self.eco_scores.items()
        ]
...
<body>
    ...
    def generate_dashboard(self):
        ...
        try:
            ...
            logger.error(
                f"Erreur de permission lors de l'écriture du dashboard: {e}", exc_info=True
            )
