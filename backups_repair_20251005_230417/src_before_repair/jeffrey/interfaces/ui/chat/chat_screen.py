...
from core.voice.voice_engine import \
    play_voice  # À créer si pas encore existant
...
from jeffrey.core.personality.conversation_personality import \
...
class ChatScreen(Screen):
    ...
    def on_pre_enter(self, *args):
        ...
        if dernier_sujet:
            self.add_jeffrey_response(
                f"Tu me parlais de « {dernier_sujet} » tout à l'heure… On reprend ? 😊"
            )
        self.tracker = ConversationTracker()
    ...
    def afficher_historique_contexte(self):
        ...
        for item in historique[-3:]:
                resume_layout.add_widget(
                    Label(text=f"• {item}", font_size="14sp", color=(0.5, 0.5, 0.5, 1))
                )
...
from __future__ import annotations
    ...
    def _journaliser_message(self, auteur, message):
        ...
        log_data.append(
            {"auteur": auteur, "message": message, "timestamp": datetime.now().isoformat()}
        )
