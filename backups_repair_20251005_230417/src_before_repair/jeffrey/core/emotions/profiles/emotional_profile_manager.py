...
from typing import Any, Dict, List, Optional, Union
    ...
    def evolve_profile_based_on_history(self, emotion_profile):
        ...
        if len(usage) >= 5:
            ...
            print(f"🔄 Profil « {emotion_profile.get('name')} » adapté selon historique (intensité {new_intensity}, délai {new_delay})")
    ...
    def _update_user_memory(self, emotion_profile):
        ...
        prefs["count"] += 1
        prefs["intensity"] = (prefs["intensity"] * (prefs["count"] - 1) + emotion_profile.get("intensity", 0.5)) / prefs["count"]
                    ...
                    if os.path.isdir(dossier):
                        for fichier in os.listdir(dossier):
                            if fichier.endswith(".json"):
                                ...
                                try:
                                    with open(chemin, "r", encoding="utf-8") as pf:
                                        data = json.load(pf)
...
from core.audio import sound_engine
from core.visual import visual_engine
...
class EmotionalProfileManager:
    ...
    def proposer_nouveau_profil(self, nom, playlist, contexte, volume=0.5, intensity=0.8, delay=1.0, shuffle=True, no_default=True, once=False, catégorie="autre", relation_context="neutre"):
    ...
    def supprimer_profil_non_valide(self, nom):
        ...
        try:
            ...
            if data.get("approved_by_david", False):
                print(f"⚠️ Impossible de supprimer : le profil est déjà validé.")
                return
                ...
                for fichier in os.listdir(dossier):
                    if fichier.endswith(".json"):
                        try:
                            with open(os.path.join(dossier, fichier), "r", encoding="utf-8") as f:
                                data = json.load(f)
                            if data.get("approved_by_david", False) and data.get("relation_context") == contexte_relationnel:
                                print(f" - {data.get('name')} ({catégorie}/{fichier})")
                        except Exception as e:
                            print(f"❌ Erreur sur {fichier} : {e}")
                ...
                for fichier in os.listdir(dossier):
                    if fichier.endswith(".json"):
                        try:
                            with open(os.path.join(dossier, fichier), "r", encoding="utf-8") as f:
                                data = json.load(f)
                            if data.get("approved_by_david", False) and data.get("relation_context") == nouveau_contexte:
        ...
        try:
            ...
            if os.path.exists(chemin):
                ...
                if data["usage_count"] >= 5:
                    print(f"🔁 Le profil « {data['name']} » est souvent utilisé ({data['usage_count']} fois). Une version évoluée va être générée automatiquement.")
                ...
                if data.get("favorite", False) and (emotion_dominante := nouveau_profil.get("playlist", [None])[0]):
                    ...
                    print(f"🧭 Jeffrey pourra le suggérer en priorité dans des contextes similaires.")
        ...
        if os.path.exists(log_path):
            try:
                ...
                if profils_possibles:
                    profil_suggéré = max(profils_possibles, key=profils_possibles.get)
                    print(f"🧠 Résurgence : Jeffrey se souvient que « {profil_suggéré} » était efficace pour « {emotion} ».")
                    ...
                    if fichier.endswith(".json"):
                        ...
                        try:
                            ...
                            if data.get("usage_count", 0) == 0 or delta > seuil_secondes:
                                statut = "jamais utilisé" if data.get("usage_count", 0) == 0 else f"inutilisé depuis {int(delta/86400)} jours"
                        ...
                        try:
                            ...
                            if data.get("usage_count", 0) == 0 or delta > seuil_secondes:
                                ...
                                if data.get("favorite", False):
                                    ...
                                    continue
                                if confirmer:
                                    print(f"❓ Supprimer « {data.get('name')} » ({catégorie}/{fichier}) ? [o/n] ", end="")
            ...
            if os.path.isdir(dossier):
                ...
                if os.path.exists(chemin):
                    try:
                        ...
                        for champ, label in [("volume", "volume (0.0–1.0)"), ("intensity", "intensité (0.0–1.0)"), ("delay", "délai (en secondes)")]:
                        ...
                        contexte = input(f"🌐 Contexte relationnel [actuel : {data.get('relation_context')}] : ").strip()
                        if contexte:
            ...
            if os.path.isdir(dossier):
                ...
                if os.path.exists(chemin):
                    try:
                        ...
                        print(f"🎛️ Volume : {data.get('volume')}, Intensité : {data.get('intensity')}, Délai : {data.get('delay')}s")
            ...
            if os.path.isdir(dossier):
                ...
                if os.path.exists(chemin):
                    try:
                        ...
                        for champ, label in [("volume", "volume (0.0–1.0)"), ("intensity", "intensité (0.0–1.0)"), ("delay", "délai (en secondes)"), ("relation_context", "contexte relationnel")]:
                            val = input(f"🔧 {label} [actuel : {data.get(champ)}] : ").strip()
                            if val:
                                try:
    ...
    def éviter_répétition_profils_tristes(self, seuil=3):
        ...
        try:
            ...
            récents = [s for s in historique[-seuil:] if s.get("emotion", "").lower() in ["tristesse", "solitude", "mélancolie"]]
    ...
    def mémoriser_relation_entre_profils(self, profil_ancien, profil_nouveau):
        ...
        try:
            ...
            relation = {
                ...
                "transition_type": "smooth" if profil_ancien.get("intensity", 0.5) < profil_nouveau.get("intensity", 0.5) else "abrupt"
            }
        ...
        try:
            ...
            if transitions:
                ...
                for t in transitions[-5:]:
                    print(f"    {t.get('from')} → {t.get('to')} ({t.get('emotion') if 'emotion' in t else t.get('emotion_to', '?')}) [{t.get('timestamp')}]")
            else:
                print("  (Aucune transition mémorisée)")
        except Exception as e:
            ...
            if os.path.isdir(dossier):
                ...
                if os.path.exists(chemin):
                    try:
                        ...
                        effet_visuel = input(f"✨ Effet visuel personnalisé (laisser vide pour conserver : {data.get('visual_effect', 'aucun')}) : ").strip()
                        if effet_visuel:
                            data["visual_effect"] = effet_visuel
                        effet_sonore = input(f"🎵 Effet sonore personnalisé (laisser vide pour conserver : {data.get('audio_effect', 'aucun')}) : ").strip()
                        if effet_sonore:
    ...
    def optimiser_transitions(self, profil_ancien, profil_nouveau, contexte=None):
        ...
        try:
            ...
            for i in range(1, étapes + 1):
                ...
                intensité = self._interpoler(profil_ancien.get("intensity", 0.5), profil_nouveau.get("intensity", 0.5), ratio)
            ...
            if os.path.isdir(dossier):
                ...
                if os.path.exists(chemin):
                    try:
                        with open(chemin, "r", encoding="utf-8") as f:
                            data = json.load(f)
                        print(f"Effets actuels : visuel = {data.get('visual_effect', 'aucun')}, audio = {data.get('audio_effect', 'aucun')}")
