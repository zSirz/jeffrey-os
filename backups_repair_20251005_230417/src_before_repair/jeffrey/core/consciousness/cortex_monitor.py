...
class CortexMonitor:
    ...
    def __init__(self, cortex: Optional[Any] = None) -> None:
        """
    ...
    def _get_memory_metrics(self) -> Dict[str, Any]:
        ...
        if hasattr(self.cortex, 'cortex') and hasattr(self.cortex.cortex, 'episodic_memory'):
            ...
            if episodic:
                now = datetime.now()
                recent_count = sum(1 for m in episodic
                                 if (now - getattr(m, 'timestamp', now)).total_seconds() < 3600)

                metrics.update({
                    "recent_memories_1h": recent_count,
                    "oldest_memory": min(getattr(m, 'timestamp', now) for m in episodic).isoformat() if episodic else None,
                    "newest_memory": max(getattr(m, 'timestamp', now) for m in episodic).isoformat() if episodic else None
                })
    ...
    def display_live_dashboard(self) -> None:
        ...
        try:
            while True:
                ...
                print(f"   RAM: {sys_metrics.get('memory_usage_mb', 'N/A'):.1f} MB ({sys_metrics.get('memory_percent', 'N/A'):.1f}%)")
