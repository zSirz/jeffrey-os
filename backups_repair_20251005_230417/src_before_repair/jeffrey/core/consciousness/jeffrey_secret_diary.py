...
from typing importDict, Optional, Tuple
...
class JeffreySecretDiary:
    ...
    def write_entry(
        self, trigger: str, emotional_state: Dict, context: Dict, intimacy_level: float = 0.5
    ) -> Dict:
        ...
        # Générer le contenu
        content = self._generate_entry_content(
            entry_type, trigger, emotional_state, context, intimacy_level
        )

        # Déterminer si c'est partageable
        shareability = self._determine_shareability(
            entry_type, content, intimacy_level, emotional_state
        )
        ...
        self.diary_entries["metadata"]["total_entries"] = (
            self.diary_entries["metadata"].get("total_entries", 0) + 1
        )
    ...
    def should_share_entry(
        self, relationship_state: Dict, context: Dict
    ...
    def _generate_entry_content(
        ...
        fill_elements = self._prepare_fill_elements(
            entry_type, trigger, emotional_state, context, intimacy_level
        )
        ...
        try:
            ...
            content = self._generate_freeform_entry(
                entry_type, trigger, emotional_state, intimacy_level
            )
    ...
    def _describe_feeling(self, emotional_state: Dict) -> str:
        """Décrit un sentiment"""
        dominant = (
            max(emotional_state.items(), key=lambda x: x[1])[0] if emotional_state else "paix"
        )
    ...
    def _add_emotional_elements(
        self, content: str, emotional_state: Dict, intimacy_level: float
    ) -> str:
    ...
    def _generate_entry_conclusion(
        self, entry_type: str, emotional_state: Dict, intimacy_level: float
    ) -> str:
    ...
    def _select_entry_to_share(self, intimacy_level: float, context: Dict) -> Optional[Dict]:
    ...
    def _record_sharing(self, entry: Dict, context: Dict):
        ...
        # Mettre à jour les métadonnées
        self.diary_entries["metadata"]["shared_count"] = (
            self.diary_entries["metadata"].get("shared_count", 0) + 1
        )
    ...
    def _time_since_last_share(self) -> Optional[timedelta]:
    ...
    def _calculate_writing_days(self) -> int:
        ...
        all_entries = self.diary_entries.get("entries", []) + self.diary_entries.get(
            "never_share", []
        )
...
if __name__ == "__main__":
    ...
    if should_share and entry_to_share:
        print(f"  Décision: Partager une entrée!")
        ...
        print(f"  Décision: Garder ses secrets pour l'instant")
