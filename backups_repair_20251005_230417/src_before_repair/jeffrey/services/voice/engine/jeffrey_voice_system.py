...
class JeffreyVoiceSystem:
    ...
    def __init__(self, config=None, emotional_core=None, test_mode=False) -> None:
        ...
        try:
            ...
            self.logger.info(
                "✅ Modules audio (sounddevice, soundfile) disponibles")
        except ImportError:
            self.logger.warning(
                "⚠️ Modules audio non disponibles. Les effets sonores seront désactivés."
            )
            print(
                "⚠️ Pour activer les effets sonores, installez : pip install sounddevice soundfile"
            )
...
import logging
    ...
    def shutdown(self):
        ...
        if self.voice_engine and hasattr(self.voice_engine, "shutdown"):
            try:
                ...
                logging.error(
                    f"[JeffreyVoiceSystem] Erreur lors de la fermeture du moteur vocal: {e}"
                )
        ...
        try:
            ...
            logging.error(
                f"[JeffreyVoiceSystem] Erreur lors de la libération des ressources audio: {e}"
            )
    ...
    def speak(
        ...
        effect_start: Optional[str] = None,
        effect_end: Optional[str] = None,
        **kwargs,
    ) -> Optional[str]:
        """
        ...
        if self.voice_mode.upper() == "OFF":
            logging.info(
                f"[JeffreyVoiceSystem] Voix désactivée. Message non prononcé: {text[:50]}..."
            )
        return None
    ...
    def apply_empathic_inversion(
        self, response: str, user_emotional_state: Dict[str, Any]
        ...
        try:
            ...
            inversion_info = self.empathic_inversion_module.process_user_emotional_state(
                user_emotional_state
            )

            # Si une inversion est nécessaire, l'appliquer à la réponse
            if inversion_info.get("inversion_needed", False) and inversion_info.get(
                "inversion_applied", False
            ):
                modified_response, voice_params = (
                    self.empathic_inversion_module.apply_empathic_inversion_to_response(
                ...
                if self.emotional_core and hasattr(
                    self.emotional_core, "appliquer_inversion_empathique"
                ):
                    _, emotional_result = self.emotional_core.appliquer_inversion_empathique(
                        response, user_emotional_state
                    )
                    # Fusionner les résultats si nécessaire
                    if emotional_result.get("success", False):
                        self.logger.info(
                            "Inversion empathique également traitée par le cœur émotionnel"
                        )
    ...
    def process_tactile_gesture(
        ...
        if self.emotional_core and hasattr(self.emotional_core, "traiter_geste_tactile"):
            try:
                ...
                self.logger.error(
                    f"Erreur lors du traitement du geste tactile via le cœur émotionnel: {e}"
                )
    ...
    def interjection(
        ...
        params_vocaux: Optional[Dict[str, Any]] = None,
    ) -> str:
        """
    ...
    def soupirer(self, type_soupir: str = "neutre", intensite: float = 0.7) -> str:
        ...
        return self.interjection(
            type_interjection=type_soupir, texte=soupir_info["texte"], params_vocaux=params_vocaux
        )

    # SPRINT 218: Intégration du module de stabilité émotionnelle

    def apply_emotional_stability_to_voice(
        self, voice_params: Dict[str, Any], current_emotion: str
    ) -> Dict[str, Any]:
        """
        ...
        if not self.emotional_core or not hasattr(
            self.emotional_core, "obtenir_etat_verrou_stabilite"
        ):
            return voice_params
        ...
        try:
            ...
            if not stability_status.get("success", False) or not stability_status.get(
                "verrou_actif", False
            ):
            ...
            if lock_level == "léger":
                ...
                if "emotion_intensity" in adapted_params:
                    adapted_params["emotion_intensity"] = max(
                        0.3, adapted_params.get("emotion_intensity", 0.5) * 0.7
                    )
                ...
                if "emotion_intensity" in adapted_params:
                    adapted_params["emotion_intensity"] = max(
                        0.2, adapted_params.get("emotion_intensity", 0.5) * 0.5
                    )
                ...
                if "emotion_intensity" in adapted_params:
                    adapted_params["emotion_intensity"] = min(
                        0.6, adapted_params.get("emotion_intensity", 0.5)
                    )
            ...
            self.logger.info(
                f"Paramètres vocaux adaptés pour respecter le verrou de stabilité (niveau: {lock_level})"
            )
            return adapted_params

        except Exception as e:
            self.logger.error(
                f"Erreur lors de l'application de la stabilité émotionnelle aux paramètres vocaux: {e}"
            )
        return voice_params  # En cas d'erreur, retourner les paramètres originaux

    # SPRINT 219: Intégration du métamoniteur émotionnel

    def adjust_voice_for_emotional_patterns(
        self, voice_params: Dict[str, Any], current_emotion: str
    ) -> Dict[str, Any]:
        """
        ...
        if not self.emotional_core or not hasattr(
            self.emotional_core, "obtenir_patterns_emotionnels"
        ):
            return voice_params
        ...
        try:
            ...
            for pattern, info in patterns.items():
                ...
                if intensity > 0.6:
                    if pattern == "oscillation":
                        ...
                        adapted_params["stability"] = min(
                            0.9, adapted_params.get("stability", 0.5) + 0.3
                        )
            ...
            self.logger.error(
                f"Erreur lors de l'ajustement vocal pour les patterns émotionnels: {e}"
            )
            return voice_params  # En cas d'erreur, retourner les paramètres originaux

    # SPRINT 220: Intégration du sélecteur de rôle émotionnel

    def apply_emotional_role_to_voice(
        self, voice_params: Dict[str, Any], current_emotion: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for trait, value in role_traits.items():
                if trait in adapted_params:
                    ...
                    role_value = value
                    adapted_params[trait] = (
                        current_value * (1 - role_intensity) + role_value * role_intensity
                    )
                else:
            ...
            self.logger.error(
                f"Erreur lors de l'application du rôle émotionnel aux paramètres vocaux: {e}"
            )
        return voice_params  # En cas d'erreur, retourner les paramètres originaux
    ...
    def start_listening(
        ...
        try:
            ...
            from orchestrateur.recognition.jeffrey_voice_recognition import (
                set_recognition_params, start_recognition_loop)
            ...
            self.logger.error(
                f"Impossible de démarrer la reconnaissance vocale: Module manquant - {e}"
            )
            print(
                f"❌ Module requis non trouvé: {str(e).split('named ')[-1] if 'named' in str(e) else str(e)}"
            )
            print(
                "   Vérifiez que le module recognition/jeffrey_voice_recognition.py est bien présent"
            )
            return False
        except Exception as e:
    ...
    def stop_listening(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Impossible d'arrêter la reconnaissance vocale: Module manquant - {e}"
            )
            return False
        except Exception as e:
                    ...
                    with mic as source:
                        ...
                        try:
                            ...
                            text = self.speech_recognizer.recognize_google(
                                audio, language=self.recognition_language
                            )
                            ...
                            print(
                                f"⚠️ Erreur de service de reconnaissance. Vérifiez votre connexion internet."
                            )
    ...
    def add_internal_state_narration(self, response: str, add_prefix: bool = False) -> str:
        ...
        try:
            ...
            if not narration_result.get("success", False) or not narration_result.get(
                "verbalisation"
            ):
    ...
    def adapt_voice_to_personality(
        self, voice_params: Dict[str, Any], current_emotion: str
    ) -> Dict[str, Any]:
        """
    ...
    def _log_voice_interaction_for_adaptation(
        self, emotion: str, voice_params: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            interaction_data = {
                ...
                "voice": {
                    "emotion": emotion,
                    "pace_preference": (
                        voice_params.get("rate", 0.5)
        if "rate" in voice_params
                        else voice_params.get("pace", 0.5)
                    ),
            ...
            self.logger.error(
                f"Erreur lors de l'enregistrement de l'interaction vocale pour l'adaptation: {e}"
            )

    def process_voice_params_for_synthesis(
        self, text: str, emotion: str, voice_params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
