...
class FaceSyncManager:
    ...
    def __init__(self, face_controller=None, voice_system=None,
    ...
    def start_sync(self):
        ...
        if self.face_controller is None:
            logger.error(
                "Impossible de démarrer la synchronisation: contrôleur facial non disponible"
            )
            return
    ...
    def _audio_event_callback(self, event_type: str, event_data: Dict[str, Any]):
        ...
        self.audio_event_queue.put(
            {"type": event_type, "data": event_data, "timestamp": datetime.now()}
        )
    ...
    def _apply_base_emotion(self, emotion: str):
        ...
        expressions = self.emotion_to_expression_map.get(
            emotion, self.emotion_to_expression_map["neutre"]
        )
    ...
    def _process_audio_chunk(self, chunk_data: Dict[str, Any]):
        ...
        try:
                    ...
                    self.face_controller.apply_micro_expression(
                        expression, intensity=intensity, duration=duration
                    )

                    logger.debug(
                        f"Micro-expression appliquée: {expression} (intensité: {intensity:.2f})"
                    )
        except Exception as e:
                    logger.error(
                        f"Erreur lors de l'application de la micro-expression '{expression}': {e}"
                    )
    ...
    def _handle_speech_pause(self, duration: float):
        ...
        try:
            ...
            if expressions:
                ...
                logger.debug(
                    f"Expression de pause appliquée: {expression} (intensité: {intensity:.2f})"
                )
        except Exception as e:
    ...
    def _generate_autonomous_expression(self):
        ...
        try:
            ...
            self.face_controller.apply_micro_expression(
                expression, intensity=intensity, duration=0.3
            )
        except Exception as e:
    ...
    def update_emotion_sync(self, current_emotion: str, emotion_intensity: float = 0.7) -> None:
        ...
        try:
            ...
            emotion_map = self.emotion_to_expression_map.get(
                current_emotion, self.emotion_to_expression_map["neutre"]
            )
            expressions = emotion_map.get(intensity_level, emotion_map["base"])

            # Appliquer progressivement les expressions
            for expression in expressions:
                self.face_controller.apply_expression(
                    expression, intensity=emotion_intensity, duration=0.8
                )
            ...
            for phoneme_data in phonemes:
                ...
                if delay > 0:
                    timer = threading.Timer(
                        delay, apply_phoneme_movement, args=[mouth_movement, duration]
                    )
                    timer.daemon = True
                    timer.start()
                else:
