...
class InterpersonalRhythmSynchronizer:
    ...
    def __init__(
        self, storage_path: Optional[str] = None, config_path: Optional[str] = None):
        """
        ...
        self.storage_path = storage_path or os.path.join(
            "data", "memory", "interaction_rhythm.json"
        )
        self.config_path = config_path or os.path.join(
            "config", "interaction_rhythms.json")
        ...
        self.logger.info(
            "InterpersonalRhythmSynchronizer initialisé avec succès")
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                if "rhythm_adaptation_policies" in config:
                    self.rhythm_adaptation_policies.update(
                        config["rhythm_adaptation_policies"])
    ...
    def _load_data(self) -> bool:
        ...
        try:
            if os.path.exists(self.storage_path):
                ...
                self.logger.info(f"Données de rythme interpersonnel chargées avec succès")
                return True
        except Exception as e:
    ...
    def adjust_jeffrey_rhythm(
        self, policy: Optional[str] = None, custom_weights: Optional[Dict[str, float]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for dimension in self.user_rhythm_model:
                ...
                target_value = (
                    weight_self * self.jeffrey_rhythm_model[dimension]
                    + weight_user * self.user_rhythm_model[dimension]
                )
                ...
                self.jeffrey_rhythm_model[dimension] = max(
                    0.0, min(1.0, self.jeffrey_rhythm_model[dimension])
                )

            # Calculer le changement global
            change_magnitude = sum(
                abs(old_model[d] - self.jeffrey_rhythm_model[d]) for d in old_model
            ) / len(old_model)
    ...
    def apply_rhythm_parameters(self, response_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "speed" in speech_params:
                    speech_params["speed"] = speech_params["speed"] * (
                        0.7 + 0.6 * self.jeffrey_rhythm_model["tempo"]
                    )

                # Pauses
        if "pause_frequency" in speech_params:
                    pause_tolerance = self.jeffrey_rhythm_model["pause_tolerance"]
                    speech_params["pause_frequency"] = speech_params["pause_frequency"] * (
                        0.5 + pause_tolerance
                    )

        if "pause_duration" in speech_params:
                    pause_tolerance = self.jeffrey_rhythm_model["pause_tolerance"]
                    speech_params["pause_duration"] = speech_params["pause_duration"] * (
                        0.5 + pause_tolerance
                    )
        ...
        if "detail_level" in result:
                ...
                result["detail_level"] = max(
                    0.1, min(1.0, result["detail_level"] * (0.5 + depth_factor))
                )

            # Ajuster la proactivité
        if "proactivity" in result:
                initiative = self.jeffrey_rhythm_model["initiative_level"]
                result["proactivity"] = max(
                    0.1, min(1.0, result["proactivity"] * (0.5 + initiative))
                )
    ...
    def get_rhythm_recommendations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            general_recommendation = self._generate_general_recommendation(
                user_prototype, sync_level, recommendations
            )
    ...
    def _extract_rhythm_metrics(self, interaction_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if message:
            ...
            avg_word_length = sum(len(word) for word in message.split()) / max(
                1, len(message.split())
            )
            complex_word_ratio = len([w for w in message.split() if len(w) > 8]) / max(
                1, len(message.split())
            )

            metrics["depth_preference"] = max(
                0.1, min(0.9, (avg_word_length / 10.0) * 0.5 + complex_word_ratio * 0.5)
            )
    ...
    def _get_response_style_recommendation(self) -> Dict[str, Any]:
        ...
        if verbosity > 0.7 and depth > 0.7:
            ...
            style = "équilibré"
            description = (
                "Réponses modérément détaillées avec un bon équilibre entre concision et richesse"
            )
    ...
    def _generate_general_recommendation(
        ...
        user_profile_desc = user_profile_descriptions.get(
            user_prototype, "un style d'interaction unique"
        )
