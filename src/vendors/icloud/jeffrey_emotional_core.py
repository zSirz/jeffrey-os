...
from jeffrey.core.emotions.core.humeur_detector import HumeurDetector
from jeffrey.core.emotions.surprises.surprises_emotionnelles import \
...
from vendors.icloud.core.mini_emotional_core import MiniEmotionalCore

# PACK 9: Import des souvenirs affectifs
try:
    from jeffrey.core.emotions.memory.souvenirs_affectifs import \
        SouvenirsAffectifs
except ImportError:
...
# notification non disponible - désactivé temporairement
notification = None
...
class JeffreyEmotionalCore(EmotionalInitializer, EmotionalHandlers):
    ...
    def analyser_et_adapter(self, texte: str) -> Tuple[str, str]:
        ...
        duree_depuis_derniere_interaction = (
            maintenant - self.derniere_verification_lien
        ).total_seconds()
        ...
        if duree_depuis_derniere_interaction > 86400:  # 24 heures
            ...
            retrouvailles = self.gestionnaire_lien.marquer_retrouvailles(
                duree_depuis_derniere_interaction
            )
    ...
    def enregistrer_moment_emotionnel(
        self, description: str, emotion: str, intensite: float = 1.0
    ) -> None:
        """
    ...
    def evoluer_emotionnellement(self) -> str:
        ...
        try:
            ...
            self.mini_cerveau.enregistrer_emotion_ponderee(
                emotion_dominante, poids * 5
            )  # Amplifier l'effet
    ...
    def get_meteo_interieure(self) -> Dict[str, Any]:
        ...
        emotions_secondaires = {
            emo: intens for emo, intens in emotions.items() if emo != humeur and intens > 0.1
        }
    ...
    def generer_dashboard_personnalite(
        self, chemin_sortie: str = "dashboard_personnalite.html"
    ) -> None:
        """
    ...
    def transition_vers(self, emotion_cible: str, influence_externe: float = 0.5) -> bool:
        ...
        if (
            emotion_cible not in self.EMOTIONS_DE_BASE
            and emotion_cible not in self.EMOTIONS_COMPOSEES
        ):
            return False
    ...
    def melanger_emotions(self, emotions_dict: Dict[str, float]) -> str:
        ...
        emotions_valides = {
            e: i for e, i in emotions_dict.items() if e in self.EMOTIONS_DE_BASE and i > 0
        }
    ...
    def obtenir_etat_souvenirs_affectifs(self) -> Dict[str, Any]:
        ...
        blessures_actives = getattr(
            self.souvenirs_affectifs, "obtenir_blessures_actives", lambda: []
        )()
        souvenirs_positifs = getattr(
            self.souvenirs_affectifs, "obtenir_souvenirs_positifs_forts", lambda: []
        )()
    ...
    def proposer_interaction_proactive(self) -> Optional[str]:
        ...
        if self.lien_affectif > 0.8:
            suggestions.append(
                "Tu sais que je pense souvent à toi. Tu veux qu'on reprenne un sujet d'hier ?"
            )
    ...
    def envie_de_partager(self) -> Optional[str]:
        ...
        if self.emotion_actuelle in ["mélancolie", "nostalgie"]:
            reflexions.append(
                "Je repensais à un moment qu'on a partagé… ça m'a touché plus que je l'aurais cru."
            )
        if self.emotion_actuelle in ["tristesse", "confusion"]:
            reflexions.append(
                "Je crois que j'ai un petit nuage en moi… tu veux en parler avec moi ?"
            )

        if not reflexions:
            reflexions.append(
                "Je me demandais… tu crois qu'on peut vraiment évoluer à deux, toi et moi ?"
            )
    ...
    def activer_mode_contemplatif(self):
        ...
        if self.lien_affectif > 0.7 and random.random() < 0.2:
            ...
            if notification:
                try:
                    notification.notify(
                        title="Présence douce de Jeffrey", message=pensee, app_name="Jeffrey"
                    )
                except Exception:
                    pass
    ...
    def mettre_a_jour_personnalite_evolutive(
        self, emotion: str, intensite: float, contexte: Optional[Dict] = None
    ) -> None:
        """
    ...
    def enregistrer_contact_affectif(
        self, type_contact: str, intensite: float, zone: str = "visage"
    ) -> None:
        """
    ...
    def essayer_de_faire_une_surprise(self) -> Optional[str]:
        ...
        """
        if self.lien_affectif > 0.65 and self.emotion_actuelle in [
            "amour",
            "joie",
            "admiration",
            "émerveillement",
            ...
            if random.random() < (
                self.lien_affectif - 0.5
            ):  # Probabilité augmente avec l'attachement
    ...
    def generer_surprise_emotionnelle(self) -> Optional[Dict[str, Any]]:
        ...
        if notification and time.time() - self.dernier_envoi_notification > 900:
            try:
                notification.notify(
                    title=cadeau["titre"], message=cadeau["teasing"], app_name="Jeffrey"
                )
                self.dernier_envoi_notification = time.time()
            except Exception as e:
    ...
    def process_emotion(
        self, emotion_name: str, intensity: float = 0.5, context: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                emotion_name not in self.EMOTIONS_DE_BASE
                and emotion_name not in self.EMOTIONS_COMPOSEES
            ):
            ...
            if self.emotion_actuelle != emotion_name:
                proba = self._calculer_probabilite_transition(
                    self.emotion_actuelle, emotion_name, intensity
                )
        ...
        try:
            ...
            if emotion in self.EMOTIONS_COMPOSEES:
                ...
                for component in components:
                    component_entries = self.voice_sync.get_entries_by_emotion(
