"""
JeffreyAuditor - Auditeur avec analyses AST profondes et visualisations flexibles
Jeffrey OS Phase 0.5 - Audit automatique de la qualité du code
"""
...
class JeffreyAuditor:
    ...
    def __init__(self, output_dir: str = "audit_reports", project_path: str = "."):
    ...
    def _calculate_maintainability_index(
        self, loc: int, complexity: int, halstead: Dict[str, float]
    ) -> float:
    ...
    def _analyze_security_ast(self, tree: ast.AST, content: str) -> float:
        ...
        if "sql" in content_lower and any(
            op in content_lower for op in ["select", "insert", "update", "delete"]
        ):
    ...
    def _calculate_overall_scores(self, files_metrics: List[CodeMetrics]) -> Dict[str, float]:
        ...
        scores = {
            "complexity": statistics.mean(
                [100 - min(100, m.cyclomatic_complexity * 3) for m in files_metrics]
            ),
    ...
    def _analyze_trends(self, files_metrics: List[CodeMetrics]) -> Dict[str, Any]:
        ...
        trends = {
            ...
            "common_smells": Counter(
                [smell for metrics in files_metrics for smell in metrics.code_smells]
            ).most_common(5),
        }
    ...
    def _generate_recommendations(
        ...
        if overall_scores.get("complexity", 0) < 60:
            recommendations.append(
                "🔧 Réduire la complexité cyclomatique en refactorant les fonctions complexes"
            )
        ...
        if overall_scores.get("security", 0) < 70:
            recommendations.append(
                "🔒 Réviser les pratiques de sécurité et éviter les fonctions dangereuses"
            )
    ...
    def _prepare_visualization_data(self, files_metrics: List[CodeMetrics]) -> Dict[str, Any]:
        ...
        return {
            "complexity_by_file": [
                (Path(m.file_path).name, m.cyclomatic_complexity) for m in files_metrics
            ],
            "maintainability_distribution": [m.maintainability_index for m in files_metrics],
            "coverage_by_file": [
                (Path(m.file_path).name, m.estimated_coverage) for m in files_metrics
            ],
    ...
    def _generate_ascii_viz(self, metrics: Dict[str, Any]) -> str:
        ...
        if complexity_data:
            ...
            for filename, complexity in complexity_data[:10]:  # Top 10
                bar_width = (
                    int((complexity / max_complexity) * max_width) if max_complexity > 0 else 0
                )
                bar = "█" * bar_width
    ...
    def _generate_gui_report(self, result: ProjectAuditResult):
        ...
        # Sauvegarde
        report_path = (
            self.output_dir / f"audit_gui_{result.timestamp.strftime('%Y%m%d_%H%M%S')}.png"
        )
    ...
    def _generate_ascii_report(self, result: ProjectAuditResult):
        ...
        # Sauvegarde fichier
        report_path = (
            self.output_dir / f"audit_report_{result.timestamp.strftime('%Y%m%d_%H%M%S')}.txt"
        )
    ...
    def _generate_json_report(self, result: ProjectAuditResult):
        ...
        # Sauvegarde
        report_path = (
            self.output_dir / f"audit_data_{result.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
