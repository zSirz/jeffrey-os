...
class JeffreyEmotionalCore(
    ...
    def __init__(self, test_mode: bool = False) -> None:
        ...
        self.gestionnaire_lien = LienAffectif(nom_utilisateur="David", empreinte_initiale=True)
        self.lien_affectif = self.gestionnaire_lien.niveau_dependance
        self.journal_lien = self.gestionnaire_lien.souvenirs_heureux

        # PACK 9: Propriétés de résonance affective
        self.resonance_affective = 0.5  # Valeur par défaut
        self.blessure_active = False  # Valeur par défaut
        ...
        from jeffrey.core.emotional_journal import EmotionalJournal
    ...
    def _load_current_state(self):
        """
        Charge l'état émotionnel actuel depuis la mémoire persistante.

        Initialise l'état émotionnel à partir du fichier de sauvegarde
        ou crée un nouvel état par défaut si aucun n'existe.
        """
        try:
            # Tenter de charger depuis le fichier de persistance
            state_file = getattr(self, 'state_file', None)

            if state_file and hasattr(state_file, 'exists') and state_file.exists():
                import json
                ...
                self.current_emotional_state = saved_state.get(
                    'emotional_state',
                    {
                        'joie': 0.5,
                        'tristesse': 0.0,
                ...
                if hasattr(self, 'emotional_history'):
                    self.emotional_history = saved_state.get('history', [])

                # Logger le chargement
                if hasattr(self, 'logger'):
                    self.logger.info(f"✅ État émotionnel chargé depuis {state_file}")
            else:
                ...
                if hasattr(self, 'logger'):
                    self.logger.info("✅ État émotionnel initialisé par défaut")

        except Exception as e:
            # En cas d'erreur, initialiser un état par défaut
            self.current_emotional_state = {
                'joie': 0.5,
                'tristesse': 0.0,
                'peur': 0.0,
            ...
            if hasattr(self, 'logger'):
                self.logger.warning(f"⚠️ Erreur lors du chargement de l'état : {e}")
                self.logger.info("✅ État émotionnel initialisé par défaut (fallback)")

    def _initialize_modules(self):
        """
        Initialise tous les modules émotionnels et leurs dépendances.

        Cette méthode configure et connecte tous les sous-systèmes
        nécessaires au fonctionnement du cœur émotionnel.
        """
        try:
            # Initialiser l'historique émotionnel
            if not hasattr(self, 'emotional_history'):
                self.emotional_history = []

            # Initialiser les compteurs et métriques
            if not hasattr(self, 'emotion_metrics'):
                self.emotion_metrics = {'total_processed': 0, 'dominant_emotions': {}, 'last_update': datetime.now()}

            # Initialiser les seuils de transition émotionnelle
            if not hasattr(self, 'transition_thresholds'):
            ...
            # Initialiser les patterns émotionnels
            if not hasattr(self, 'emotional_patterns'):
                self.emotional_patterns = {'morning_boost': 0.1, 'evening_calm': 0.15, 'social_amplifier': 0.2}

            if hasattr(self, 'logger'):
                self.logger.info("✅ Modules émotionnels initialisés avec succès")

        except Exception as e:
            if hasattr(self, 'logger'):
                self.logger.warning(f"⚠️ Erreur lors de l'initialisation des modules : {e}")

    def analyze_emotion_hybrid(self, text: str, context: dict = None) -> dict:
        """
        Analyse hybride d'émotions utilisant plusieurs méthodes de détection.

        Combine la détection par mots-clés, emojis et patterns contextuels
        pour une analyse émotionnelle robuste et précise.

        Args:
            text (str): Texte à analyser
            context (dict): Contexte additionnel pour l'analyse

        Returns:
            dict: Résultat de l'analyse avec émotion, intensité, confiance
        """
        if not text or not isinstance(text, str):
            return {'emotion': 'neutre', 'intensity': 50, 'confidence': 0.1, 'method': 'fallback'}
        ...
        try:
            ...
            emoji_emotions = self._detect_from_emojis(text)
            if emoji_emotions:
                emotions_detected.update(emoji_emotions)

            # 2. Détection par mots-clés
            keyword_emotions = self._detect_from_keywords(text)
            if keyword_emotions:
                for emotion, score in keyword_emotions.items():
                    emotions_detected[emotion] = emotions_detected.get(emotion, 0) + score

            # 3. Analyse contextuelle
            if context:
                context_boost = self._analyze_context(context)
                for emotion, boost in context_boost.items():
                    emotions_detected[emotion] = emotions_detected.get(emotion, 0) + boost

            # 4. Déterminer l'émotion dominante
            if emotions_detected:
                dominant_emotion = max(emotions_detected, key=emotions_detected.get)
                intensity = min(100, int(emotions_detected[dominant_emotion] * 100))
                confidence = min(1.0, len(emotions_detected) * 0.2 + 0.3)
            else:
                # Fallback : analyse basique
                dominant_emotion = self._basic_sentiment_analysis(text)
                intensity = 65
                confidence = 0.4

            # Mettre à jour l'état émotionnel
            self._update_emotional_state(dominant_emotion, intensity / 100.0)

            # Enregistrer dans les métriques
            self.emotion_metrics['total_processed'] += 1
            ...
            self.last_processed_emotion = result
            return result

        except Exception as e:
            if hasattr(self, 'logger'):
                self.logger.warning(f"Erreur analyse émotionnelle: {e}")
    ...
    def _detect_from_emojis(self, text: str) -> dict:
        """Détecte les émotions à partir des emojis dans le texte."""
        emoji_map = {
            '😊': {'joie': 0.8},
            '😄': {'joie': 0.9},
            '🎉': {'joie': 0.85, 'enthousiasme': 0.7},
            '✨': {'joie': 0.6, 'émerveillement': 0.5},
            '😢': {'tristesse': 0.8},
            '😭': {'tristesse': 0.9},
            '😡': {'colère': 0.9},
            '😤': {'colère': 0.7, 'frustration': 0.6},
            '😨': {'peur': 0.8},
            '😰': {'peur': 0.7, 'anxiété': 0.6},
            '❤️': {'amour': 0.9},
            '💖': {'amour': 0.8, 'affection': 0.7},
            '🤔': {'curiosité': 0.6, 'réflexion': 0.5},
            '😌': {'sérénité': 0.8},
            '😴': {'fatigue': 0.7, 'calme': 0.5},
        }

        emotions = {}
        for emoji, emotion_dict in emoji_map.items():
            if emoji in text:
    ...
    def _detect_from_keywords(self, text: str) -> dict:
        """Détecte les émotions à partir des mots-clés."""
        text_lower = text.lower()
        keyword_map = {
            'joie': ['heureux', 'content', 'ravi', 'joyeux', 'super', 'génial', 'fantastique', 'merveilleux'],
            'tristesse': ['triste', 'malheureux', 'déprimé', 'mélancolique', 'peine', 'chagrin'],
            'colère': ['énervé', 'furieux', 'agacé', 'irrité', 'fâché', 'rage'],
            'peur': ['peur', 'effrayé', 'anxieux', 'inquiet', 'stressé', 'paniqué'],
            'amour': ['amour', 'adore', 'affection', 'tendresse', 'chéri', 'aimé'],
            'curiosité': ['curieux', 'intéressant', 'pourquoi', 'comment', 'découvrir'],
            'sérénité': ['calme', 'paisible', 'serein', 'tranquille', 'zen', 'apaisé'],
        }
    ...
    def _analyze_context(self, context: dict) -> dict:
        """Analyse le contexte pour ajuster les émotions."""
        context_boost = {}

        if not context:
            return context_boost

        # Analyse du moment de la journée
        time_of_day = context.get('time_of_day', '')
        if time_of_day == 'morning':
            context_boost['joie'] = context_boost.get('joie', 0) + 0.1
        elif time_of_day == 'evening':
            context_boost['sérénité'] = context_boost.get('sérénité', 0) + 0.1

        # Analyse de l'humeur récente
        recent_mood = context.get('recent_mood', '')
        if recent_mood:
            context_boost[recent_mood] = context_boost.get(recent_mood, 0) + 0.2

        return context_boost

    def _basic_sentiment_analysis(self, text: str) -> str:
        """Analyse de sentiment basique en fallback."""
        positive_words = ['bien', 'bon', 'super', 'ok', 'oui', 'merci', 'bravo']
        negative_words = ['mal', 'non', 'problème', 'erreur', 'difficile']
    ...
    def _update_emotional_state(self, emotion: str, intensity: float):
        """Met à jour l'état émotionnel interne."""
        if hasattr(self, 'current_emotional_state'):
            # Ajuster l'émotion dans l'état
            if emotion in self.current_emotional_state:
                current = self.current_emotional_state[emotion]
                # Moyenne pondérée avec la nouvelle intensité
                self.current_emotional_state[emotion] = (current * 0.7) + (intensity * 0.3)
            else:
                self.current_emotional_state[emotion] = intensity * 0.5

            # Normaliser les valeurs
            for key in self.current_emotional_state:
                self.current_emotional_state[key] = max(0.0, min(1.0, self.current_emotional_state[key]))
