...
PLUGIN_INFO = {
    ...
    "config": {"auto_start": True, "enable_memory": True, "enable_emotions": True},
}
...
class OrchestratorBridge(ServiceModule):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self._emotional_core: EmotionalCore | None = None
        ...
        try:
            ...
            if event_bus:
                ...
                self._subscribed_events.extend(
                    [Events.USER_MESSAGE, Events.EMOTION_CHANGED, 'identity_needs_confirmation']
                )
    ...
    async def process_message(self, message: str, session_id: str = "default") -> str:
        ...
        try:
            ...
            response = self._dialogue_engine.generate_response(message, emotional_context.get("emotion", "neutral"))
        ...
        try:
            ...
            if self._orchestrator:
                ...
                dialogue_response = self._dialogue_engine.generate_response(
                    enriched_message, emotion_state.get("primary_emotion") if emotion_state else "neutral"
                )
                ...
                response = {
                    ...
                    "metadata": {
                        ...
                        "context_used": bool(
                            memory_context.get("recent_conversation") or memory_context.get("emotional_memories")
                        ),
                    ...
                    "enriched_input": enriched_message
                    if len(enriched_message) < 1000
                    else enriched_message[:1000] + "...",
                    "context_info": {
                        "has_history": bool(memory_context.get("recent_conversation")),
                        "has_emotions": bool(memory_context.get("emotional_memories")),
                        "memories_count": len(memory_context.get("recent_conversation", []))
                        + len(memory_context.get("emotional_memories", [])),
                        "user_memory": None,  # TODO: Fix memory retrieval
                    },
                }
            ...
            await self.after_request({"message": message, "session_id": session_id}, response)
    ...
    async def _on_user_message(self, event: Event):
        ...
        if self._orchestrator:
            ...
            response = await asyncio.to_thread(self._orchestrator.process, enriched_input)
            ...
            await event_bus.emit(
                Event(
                    Events.ASSISTANT_RESPONSE,
                    {"content": response, "session_id": session_id, "metadata": {"enriched": True}},
                    source="orchestrator_bridge",
                )
            )
    ...
    async def get_enhanced_context(self, session_id: str = "default") -> dict[str, Any]:
        ...
        context = {"conversation": [], "emotional_state": None, "emotional_context": None, "recent_patterns": []}
    ...
    async def search_memories(self, query: str, session_id: str | None = None) -> dict[str, Any]:
        """Search through enhanced memories"""
        results = {"conversations": [], "emotional_memories": []}

        # Search conversations
        if self._conversation_memory:
            results["conversations"] = await self._conversation_memory.search_conversations(query, session_id)
    ...
    async def _update_plugins_from_dialogue_engine(self):
        ...
        try:
            # TODO: Fix memory retrieval
            user_memory = {}

            # Mettre à jour le UserIdentityManager
            if self._user_identity_manager and user_memory.get('name'):
                await self._user_identity_manager.confirm_identity(user_memory['name'])
            ...
            if self._emotion_adjuster and user_memory.get('emotional_context'):
                await self._emotion_adjuster.adjust_emotions(user_memory['emotional_context'])
