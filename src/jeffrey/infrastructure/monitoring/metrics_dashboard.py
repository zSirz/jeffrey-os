...
try:
    ...
    from rich.progress import BarColumn, Progress, SpinnerColumn, TaskProgress, TextColumn
...
class TerminalDashboard:
    ...
    async def _run_rich_dashboard(self) -> None:
        ...
        try:
            with Live(
                self._create_rich_layout(), refresh_per_second=1 / self.config.refresh_rate, console=self.console
            ) as live:
                while self.running:
                    # Update data
    ...
    def _create_rich_layout(self) -> Layout:
        ...
        if self.current_mode == DashboardMode.OVERVIEW:
            layout.split_column(
                ...
                Layout().split_row(
                    Layout(self._create_system_metrics_panel()), Layout(self._create_performance_panel())
                ),
                Layout().split_row(Layout(self._create_health_status_panel()), Layout(self._create_alerts_panel())),
    ...
    def _create_header_panel(self) -> Panel:
        ...
        return Panel(Align.center(header_text + "\n" + mode_text), title="🚀 Jeffrey OS v0.6.1", border_style="blue")
    ...
    def _create_performance_panel(self) -> Panel:
        ...
        if len(self.cpu_history) > 1:
            content += ASCIIChart.line_chart(self.cpu_history[-20:], width=40, height=6, title="CPU Usage (20 samples)")
            content += "\n\n"

        if len(self.response_time_history) > 1:
            content += ASCIIChart.line_chart(
                self.response_time_history[-20:], width=40, height=6, title="Response Time (ms)"
            )
    ...
    def _create_health_status_panel(self) -> Panel:
        ...
        if self.health_checker:
            ...
            status_emoji = {'healthy': '🟢', 'degraded': '🟡', 'unhealthy': '🟠', 'critical': '🔴'}.get(status, '⚪')
    ...
    def export_snapshot(self, filename: str | None = None) -> str:
