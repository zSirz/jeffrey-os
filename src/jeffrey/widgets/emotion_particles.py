...
class EmotionParticles(Widget):
    """
    Widget that displays emotion particles for UI integration.
    This is the main class imported by screen files.
    """

    emotion = StringProperty("neutral")
    intensity = NumericProperty(0.5)
    active = NumericProperty(0)  # 0 for inactive, 1 for active
    color = ListProperty([1, 1, 1, 1])

    def __init__(self, **kwargs):
        super(EmotionParticles, self).__init__(**kwargs)
        self.emitter = EmotionParticleEmitter()
        self.add_widget(self.emitter)
        self._scheduled_emission = None
    ...
    def on_active(self, instance, value):
        ...
        if value > 0:
            ...
            self._unschedule_emission()

    def _schedule_emission(self):
        """Schedule regular particle emission."""
        if not self._scheduled_emission:
            self._scheduled_emission = Clock.schedule_interval(self._emit_particles, 0.5)

    def _unschedule_emission(self):
        """Unschedule particle emission."""
        if self._scheduled_emission:
            Clock.unschedule(self._scheduled_emission)
            self._scheduled_emission = None

    def _emit_particles(self, dt):
        """Emit particles based on current emotion and intensity."""
        emotion_map = {
            "joy": "happy",
            "joie": "happy",
            "happiness": "happy",
        ...
        emotion_type = emotion_map.get(self.emotion.lower(), "neutral")
        self.emitter.emit_particle(emotion_type, self.intensity)

    def update_particles(self):
        """Update particles based on current state."""
        if self.active > 0:
            self._emit_particles(0)
